...
export default class MegaMediaSource {
  ...
  sourceOpen() {
    this.readInitSegment().then(([data, segment]) => {
      ...
      this.sourceBuffer = this.mediaSource.addSourceBuffer(MegaMediaSource.getCodec(data))
  ...
  seeking() {
    this.seekQueue = this.seekQueue.then(_ => new Promise(resolve => {
      console.log('Seeking');
      let time = this.video.currentTime;
      if (this.lastSeek == time)
        return resolve();
      this.lastSeek = time;
      ...
      this.file.download({ start: this.lastByte, end: next.offset }, (err, data) => {
        this.appendBuffer(data);
        const next = this.cues.slice(0).reverse().find(e => e.time / 1000 <= time);
  ...
  static getCodec({ Segment: { Tracks }}) {
    ...
    for (const track of Tracks) {
      ...
      if (codec.startsWith('v_'))
        vCodec = codec.slice(2);
      else if (codec.startsWith('a_'))
        aCodec = codec.slice(2);
    }
    if (vCodec && aCodec)
      return `video/webm;codecs="${vCodec},${aCodec}"`;
    else if (vCodec)
      return `video/webm;codecs="${vCodec}"`;
    else if (aCodec)
      return `audio/webm;codecs="${aCodec}"`;
  }
  ...
  static getCues({ Segment: { dataOffset, Cues, Info: { TimecodeScale } }}) {
    ...
    for (const cue of Cues) {
      cues.push({
        ...
        offset: byteArrayToLong(cue["CueTrackPositions"]["CueClusterPosition"].data) + dataOffset
      })
    }
    return cues;
  }
